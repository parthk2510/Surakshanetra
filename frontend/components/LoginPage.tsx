...
import { Lock, User, Mail, AlertCircle, Eye, EyeOff, Shield, CheckCircle, XCircle, ArrowLeft } from 'lucide-react';
...
const LoginPage = ({ onLoginSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Handle server errors before attempting JSON parse
            if (response.status >= 500) {
                structuredLogger.warn(`auth.server_error`, `Backend returned ${response.status}`, {});
            ...
            try {
                ...
                structuredLogger.error('auth.parse_error', 'Invalid JSON from server', { error: parseError, status: response.status });
